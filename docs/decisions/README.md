...
| [ADR-017](ADR-017-rfp-approval-any-staff.md) | RFP Approval Performable by Any Staff User (MVP) | Accepted | Any staff user may perform every RFP lifecycle transition; the Draft → Approved → Published sequence is retained as the extension point for future separation of duties. |
| [ADR-018](ADR-018-no-pre-moderation.md) | No Pre-Moderation of Expat-Submitted Content (MVP) | Accepted | Content is visible per its visibility rules immediately; private-by-default plus staff deletion serve as the safety valve, with flagging/review gates as future escalations. |
| [ADR-019](ADR-019-english-only-i18n-ready.md) | English-Only, No i18n Investment | Accepted (revised) | English-only across both apps with no string externalization; a future second-language requirement triggers an agent-executed retrofit pass across both apps instead. |
| [ADR-020](ADR-020-identity-separation-and-invitation-flow.md) | Identity Separation and Local-Account Invitation Flow | Proposed | One External ID tenant per environment; separate app registrations and user flows per audience (staff = email+password, portal = Google); replace B2B `/invitations` with Graph local-account creation plus app-sent invitation email and SSPR. |
